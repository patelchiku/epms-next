...
export default function AddDealPage() {
  ...
  const { register, handleSubmit, control } = useForm<any>({
    defaultValues: {
      payments: [{ amount: "", date: "", remark: "" }],
    },
  });
  const { fields, append, remove } = useFieldArray({ control, name: "payments" });

  useEffect(() => {
    axios.get("/api/users?active=true").then((r) => setUsers(r.data.data || r.data)).catch(() => {});
  }, []);
  ...
  async function onSubmit(data: any) {
    ...
    try {
      ...
      await axios.post("/api/deals", { ...data, payments });
  ...
  return (
    <div>
      <Header
        title="Add Property Deal"
        breadcrumbs={[{ label: "Dashboard", href: "/dashboard" }, { label: "Deals", href: "/deals" }, { label: "Add" }]}
      />
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="form-section space-y-5">
            ...
            <div className="form-grid">
              ...
              <div>
                ...
                <select {...register("employeeId", { valueAsNumber: true })} className="select">
          ...
          <div className="flex items-center gap-3">
            <button type="submit" disabled={saving} className="btn-primary px-8">
              {saving ? <><Loader2 className="w-4 h-4 animate-spin" />Saving...</> : "Save Deal"}
            </button>

...
export default function EditPropertyPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      reset({
        ...
        sourceId: p.sourceId ?? "",
        remark: p.remark ?? "",
        otherMobiles: mobiles.length ? mobiles.map((v: string) => ({ value: v })) : [{ value: "" }],
      });
  ...
  return (
    <div>
      <Header
        title="Edit Property"
        breadcrumbs={[{ label: "Dashboard", href: "/dashboard" }, { label: "Properties", href: "/properties" }, { label: "Edit" }]}
      />
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            <button type="submit" disabled={saving} className="btn-primary px-8">
              {saving ? <><Loader2 className="w-4 h-4 animate-spin" />Saving...</> : "Update Property"}
            </button>
